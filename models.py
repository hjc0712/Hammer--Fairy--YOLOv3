...
def create_modules(module_defs):
    ...
    for idx, module_def in enumerate(module_defs):
        ...
        if module_type == "convolutional":
            ...
            has_bn = int(module_def["batch_normalize"])
            pad = int(module_def["pad"])

            bias = not has_bn
            padding = (kernel_size - 1) // 2 if pad else 0

            if len(output_filters_list) == 0:
                prev_filters = int(net_info["channels"])
            else:
                prev_filters = output_filters_list[-1]

            conv = nn.Conv2d(prev_filters, filters, kernel_size, stride, padding=padding, bias=bias)
            modules.add_module(f"conv_{idx}", conv)

            if has_bn:
                bn = nn.BatchNorm2d(filters, momentum=0.9, eps=1e-5)
                modules.add_module(f"batch_norm_{idx}", bn)
            if module_def["activation"] == "leaky":
                activation = nn.LeakyReLU(0.1, inplace=True)
                modules.add_module(f"leaky_{idx}", activation)
            ...
            filters = sum([output_filters_list[i] for i in layers])
            modules.add_module(f"route_{idx}", EmptyLayer())

        elif module_type == "shortcut":
            filters = output_filters_list[int(module_def["from"])]
            ...
            anchors = [(anchors[i * 2], anchors[i * 2 + 1]) for i in range(0, len(anchors) // 2)]
        ...
        output_filters_list.append(filters)
...
class Darknet(nt.NeuralNetwork):
    ...
    def forward(self, x, targets=None):
        img_dim = x.shape[2]
        layer_outputs, detections = [], []
        loss = 0

        for idx, module_def in enumerate(self.module_defs):
            module_type = module_def["type"]
    ...
    def criterion(self, detections, targets): 
        return self.loss
