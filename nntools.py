...
class Experiment(object):
    ...
    def __init__(self, net, train_set, val_set, optimizer, stats_manager, checkpoint_interval,
        ...
        train_loader = td.DataLoader(train_set, batch_size=batch_size, shuffle=True,
                                     drop_last=True, pin_memory=True, collate_fn=train_set.collate_fn)
        ...
        checkpoint_path = os.path.join("checkpoints", output_dir)
        os.makedirs(checkpoint_path, exist_ok=True)
        config_path = os.path.join(checkpoint_path, "config.txt")
    ...
    def save(self, epoch):
        ...
        torch.save(self.state_dict(), f"%s/yolov3_ckpt_%d.pth" % (self.checkpoint_path, epoch))
    ...
    def run(self, num_epochs, plot=None, log=None):
        ...
        for epoch in range(start_epoch, num_epochs):
            s = time.time()
            self.stats_manager.init()
            for batch_i, (_, x, d) in enumerate(self.train_loader):
                x, d = Variable(x.to(self.net.device)), Variable(d.to(self.net.device), requires_grad=False)
                self.optimizer.zero_grad()
                y = self.net.forward(x, d)
                ...
                if log is not None:
                    log(self, loss, epoch, num_epochs, batch_i, len(self.train_loader), s)
