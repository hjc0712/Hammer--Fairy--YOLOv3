...
def pad_to_square(img, pad_val):
  ...
  img = F.pad(img, padding, value=pad_val)
  return img, padding
...
class ImageFolder(Dataset):
    def __init__(self, folder_path, img_size=416):
        self.img_paths = sorted(glob.glob("%s/*.*" % folder_path))
        self.img_size = img_size

    def __getitem__(self, index):
        img_path = self.img_paths[index]
        img = resize(pad_to_square(transforms.ToTensor()(Image.open(img_path)), 0)[0], self.img_size)
